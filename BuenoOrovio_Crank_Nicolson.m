...
%
% Using a second-order implicit scheme (Crank-Nicolson)
%
...
for i=2:steps
    ...
    while abs(Res)>1e-7 && iter < 14
        ...
        if (u1>thev)
            ...
            v1      =  (2*tauvplus-dt)*v/(dt+2*tauvplus);
            w1      =  (2*tauwplus-dt)*w/(dt+2*tauwplus);
            s1      =  (s*(4*taus2-2*dt)+(tanh(ks*(u-us))+tanh(ks*(u1-us))+2)*dt)/(4*taus2+2*dt); 
            Isi     =  - w1*s1/tausi;
            Iso     =  1/tauso;
        ...
        elseif (u1>thew)
            ...
            v1      =  (2*tauv2min-dt)*v/(dt+2*tauv2min);
            if (u1<thevmin); v1 = (2*dt + (2*tauv1min-dt)*v)/(dt+2*tauv1min); end;
            w1      =  (2*tauwplus-dt)*w/(dt+2*tauwplus);
            s1      =  (s*(4*taus2-2*dt)+(tanh(ks*(u-us))+tanh(ks*(u1-us))+2)*dt)/(4*taus2+2*dt); 
            Isi     =  - w1*s1/tausi;
            Iso     =  1/tauso; 
        ...
        elseif (u1>theo)
            ...
            v1      =  (2*tauv2min-dt)*v/(dt+2*tauv2min);
            if (u1<thevmin); v1 = (2*dt + (2*tauv1min-dt)*v)/(dt+2*tauv1min); end;
            w1      =  ((2*tauwmin-dt)*w+2*dt*winfast)/(dt+2*tauwmin);
            s1      =  (s*(4*taus1-2*dt)+(tanh(ks*(u-us))+tanh(ks*(u1-us))+2)*dt)/(4*taus1+2*dt); 
            Isi     =  0;
            Iso     =  (u1-uo)/tauo2;
            Ifi     =  0;
            dIt     =  1/tauo2;
            ...
            v1      =  (2*dt + (2*tauv1min-dt)*v)/(dt+2*tauv1min); 
            w1      =  ((2*tauwmin-dt)*w+2*dt*(-(u1/tauwinf)+1))/(dt+2*tauwmin);
            s1      =  (s*(4*taus1-2*dt)+(tanh(ks*(u-us))+tanh(ks*(u1-us))+2)*dt)/(4*taus1+2*dt); 
            Isi     =  0;
            Iso     =  (u-uo)/tauo1;
            Ifi     =  0;
        ...
        Res         =  (u1 - u)/dt + 0.5*(Isi + Iso + Ifi + Istim + In1);
