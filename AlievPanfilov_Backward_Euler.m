...
for i=2:steps
    ...
    while abs(Res)>1e-10 && iter < 14  
        ...
        aux         =  (1 + dt/scale*(epsil + beta*k*u1*(u1-alpha-1)/(u1+gamma)));
        v1          =  (-aux + sqrt(aux^2 - 4*dt/scale*beta/(u1+gamma)*(dt/scale*epsil*k*u1*(u1-alpha-1)- v)))/(2*dt/scale*beta/(u1+gamma));
        dvdu        =  beta*v1/(u1+gamma)*(v1 + k*u1*(u1-alpha-1)) - k*(1*u1-alpha-1)*(epsil*(u1+gamma)+beta*v1)/(beta*(v1+k*u1*(u1-alpha-1)) + (2/dt/scale+epsil)*(u1+gamma));
